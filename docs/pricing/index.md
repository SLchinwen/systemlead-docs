---
product: company
docType: overview
title: 定價與報價總覽
audience:
  - sales
  - cs
  - product
  - finance
  - management
---

## 定價與報價

本區彙整 **公司報價策略、定價治理與業務報價流程**，  
用於協助業務、客服、產品與管理層在報價與定價決策上保持一致與合規。

> **文件性質：非 SSOT（說明文件）**  
> 本區為內部參考與作業指引。具體報價核准、折扣權限等，以公司正式公告為準。

---

## 文件導覽

| 類型 | 文件 | 適用對象 | 用途 |
| ---- | ---- | -------- | ---- |
| **資料** | [級距與產品代碼 (pricing-tiers.yaml)](./pricing-tiers.yaml) | 產品、財務、開發、業務 | 產品命名與級距錨點**單一維護來源** |
| **參考** | [級距公式與維護說明](./pricing-tier-formula.md) | 產品、財務、開發 | 輸入千張算出年費、分段線性公式、維護流程 |
| **治理** | [定價治理辦法](./pricing-governance.md) | 產品、財務、營運、管理層 | 標準服務定價框架、檢討節奏、折扣治理 |
| **治理** | [專案報價治理辦法](./project-quoting-governance.md) | 業務、產品、財務、管理層 | 專案／混合報價原則、工時治理、報價預算書結構 |
| **範本** | [報價預算書範本](./project-quote-budget-template.md) | 業務、客服 | 專案／混合報價之客戶端範本，可複製使用 |
| **範本** | [客製專案報價標準範本](./custom-project-quote-standard-template.md) | 業務 | 混合報價標準結構、填寫指引、查表項目、計算原則 |
| **引導** | [線上報價產品引導](./online-quoting-product-guide.md) | 業務、客服、客戶 | 線上首購表單步驟、費用項目、選購模組、客服話術 |
| **QA** | [業務報價 QA](./sales-quoting-qa.md) | 業務、客服 | 報價常見問題、標準說法、競品對應 |
| **SOP** | [業務報價 SOP](./sop-sales-quoting.md) | 業務、客服 | 標準方案報價流程、檢核清單、核准流程 |

---

## 適用範圍

- **標準服務**：e首發票線上／線下標準方案（年度張數、選購模組）→ 定價治理辦法、業務報價 SOP
- **專案／混合報價**：非線上報價、客製開發、客製 + 發票模組 → [專案報價治理辦法](./project-quoting-governance.md)、[報價預算書範本](./project-quote-budget-template.md)

---

## 快速連結

### 業務/客服常用

- [客製專案報價標準範本（含填寫指引）](./custom-project-quote-standard-template.md)  
  混合報價標準結構、查表項目、計算原則、資料不足時詢問清單
- [報價預算書範本（專案／混合報價）](./project-quote-budget-template.md)  
  可複製使用，含工時估算、混合報價結構、銷售說明
- [專案報價治理辦法](./project-quoting-governance.md)  
  專案與混合報價原則、必備結構、工時治理
- [線上報價產品引導（首購表單步驟與項目）](./online-quoting-product-guide.md)  
  方案選擇、Step 對照、選購模組、客服話術
- [報價前必區分：資料來源與取號規則](./pricing-governance.md#05-報價前必區分資料來源與取號規則)  
  訂單版／發票版／混合使用、為何發票版費用較高、混合一律發票版報價
- [對客戶的說法：為何採年度張數計費](./pricing-governance.md#1-對客戶的說法為何採年度張數計費)  
  一句話版、30 秒版、長版說法
- [客戶常見 Q&A](./sales-quoting-qa.md#客戶常見-qa)  
  可直接給客服或客戶的標準回答
- [業務報價 SOP 流程](./sop-sales-quoting.md#二報價流程)  
  從需求確認到報價送出

### 管理/產品/財務／開發

- [級距公式與維護說明](./pricing-tier-formula.md)  
  輸入張數（千張）→ 年費；分段線性內插；維護僅改 YAML
- [級距與產品代碼資料 (pricing-tiers.yaml)](./pricing-tiers.yaml)  
  產品 id／name、各方案錨點（千張 → 年費）單一來源
- [產品方案標準命名](./pricing-governance.md#02-產品方案標準命名報價表單系統一致)  
  plan_order／plan_invoice／plan_overseas／plan_kiosk 對照表
- [定價治理目標與分層](./pricing-governance.md#31-治理目標)
- [年度定價檢討節奏](./pricing-governance.md#33-年度定價檢討節奏建議)
- [折扣治理規則](./pricing-governance.md#35-決策規則建議採用)
